#!/usr/bin/env python
# mpirun -np 4 python main_all.py -l2 6e-7 -n_epochs 2 -warm 2 -prune 1 -sparse 0.90  -prune_deep 1 -prune_fm 1 -prune_r 1 -use_fwlw 1 -emb_r 0.444 -emb_corr 1. -batch_size 2048
import os
import random
import argparse
...
parser.add_argument('-hook', default='No', type=str, help='DDP Hook')
...
with torch.cuda.device(rank):
    ...
    if pars.use_cuda:
        ...
        model.fit(result_index[rank*local_num:(rank+1)*local_num], result_value[rank*local_num:(rank+1)*local_num], result_label[rank*local_num:(rank+1)*local_num], test_index, test_value, test_label, \
                prune=pars.prune, prune_fm=pars.prune_fm, prune_r=pars.prune_r, prune_deep=pars.prune_deep, save_path=save_model_name, emb_r=pars.emb_r, emb_corr=pars.emb_corr,rank=rank, device=rank, distributed=distribute, hook = pars.hook)
