...
class DeepFMs(torch.nn.Module):
    ...
    def fit(self, Xi_train, Xv_train, y_train, Xi_valid=None, Xv_valid=None, 
            y_valid = None, ealry_stopping=False, refit=False, save_path = None, prune=0, prune_fm=0, prune_r=0, prune_deep=0, emb_r=1., emb_corr=1., rank=0, device=0, distributed=False, hook=None):
        """
        ...
        is_valid = False
        Xi_train = Xi_train.reshape((-1, self.field_size-self.num, 1))
        #Xv_train = np.array(Xv_train)
        #y_train = np.array(y_train)
        x_size = Xi_train.shape[0]
        if len(Xi_valid) > 0:
            Xi_valid = Xi_valid.reshape((-1, self.field_size-self.num, 1))
        ...
        if distributed:
            ...
            if hook =="default":
                print("Default Hook")
                model.register_comm_hook(None, default_hook)
            elif hook =="standard_dithering":
                print("Standard Dithering Hook")
                model.register_comm_hook(None, standard_dithering_hook)
                ...
                print("LGreco Hook")
                state = lgreco_hook.PowerSGDState(torch.distributed.group.WORLD,
                            matrix_approximation_rank=4,
                            start_powerSGD_iter=2,
                            adjust_freq=1000,
                            adjust_gamma=0.25,
                            adjust_beta=4)
                model.register_comm_hook(state, lgreco_hook.powerSGD_hook)
            elif hook =="powersgd":
                print("powersgd Hook")
                state = powerSGD_hook.PowerSGDState(torch.distributed.group.WORLD,
                                           matrix_approximation_rank=4,
                                           start_powerSGD_iter=2)
        ...
        for epoch in range(self.n_epochs):
            ...
            for i in range(batch_iter+1):
                ...
                if self.verbose and i % 100 == 99:
                    ...
                    # print('[%d, %5d] loss: %.6f metric: %.6f time: %.6f s' %
                    #       (epoch + 1, i + 1, total_loss/100.0, eval, time()-batch_begin_time))
                    #if rank==0:
                    #    wandb.log({"Test loss":total_loss/100.0})
                    #    wandb.log({"Test AUC":eval})
                    #    wandb.log({"Test batch time":time()-batch_begin_time})
                    #    wandb.log({"Test throughput":self.batch_size/(time()-batch_begin_time)})
                    total_loss = 0.0
                    batch_begin_time = time()
            ...
            if is_valid:
                ...
                print('Validation [%d] loss: %.6f metric: %.6f sparse %.2f%% time: %.6f s' %
