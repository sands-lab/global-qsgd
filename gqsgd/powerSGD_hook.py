...
class PowerSGDState(object):
    ...
    def __init__(
        ...
        self.rng = np.random.RandomState(random_seed) ## pass the random seed here, same as train script
...
def powerSGD_hook(
    ...
    if not need_randomize_qs:
        ...
        with torch.random.fork_rng(devices=[]):
            ...
            torch.manual_seed(state.rng.randint(1_000_000_000)) ## same seed as script ## state.flag to do it once
            for q in qs:
                q.copy_(
                    torch.randn(
                        *q.shape,
